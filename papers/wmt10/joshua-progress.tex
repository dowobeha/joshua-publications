...
translation introduced by \newcite{samt2006} aims at improving
...
\end{tabular}
\end{center}
extracted from the bitext, the nonterminals are labeled according to
which constituents cover the nonterminal span on the parsed side of
the bitext.
\begin{center}
\begin{tabular}{c}
  {\small \tt [VP]} $\Rightarrow$ {\small \tt [VBN,1]} sans {\small
    \tt [NP,2]} $\vert$ $\mathit{VBN}_1$ without {\small \tt [NP,2]} \\
  {\small \tt [NP]} $\Rightarrow$ {\small \tt [NP,1]} sans {\small
    \tt [NP,2]} 
  $\vert$ {\small \tt [NP,1]} without {\small \tt [NP,2]} 
\end{tabular}
\end{center}
To ensure good coverage, the SAMT approach concatenates syntactic
constituents or uses CCG-style labels when there is no single
constituent spanning a nonterminal. The derivations licensed by the
resulting grammar are constrained to match the syntactic structure,
typically leading to more consistent reorderings.

\joshua's decoder supports SAMT-style grammars as well as non-syntactic
nonterminal annotations obtained from other sources, such as named
entity taggers. 
